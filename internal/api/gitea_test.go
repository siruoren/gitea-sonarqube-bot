...
func withValidGiteaCommentRequestData(t *testing.T, jsonBody []byte) (*http.Request, *httptest.ResponseRecorder, http.HandlerFunc) {
...
func withValidGiteaSynchronizeRequestData(t *testing.T, jsonBody []byte) (*http.Request, *httptest.ResponseRecorder, http.HandlerFunc) {
	webhookHandler := NewGiteaWebhookHandler(new(GiteaSdkMock), new(SQSdkMock))
...
func TestHandleGiteaCommentWebhookInvalidJSONBody(t *testing.T) {
	...
	req, rr, handler := withValidGiteaCommentRequestData(t, []byte(`{ "action": ["non-string-value-for-action"] }`))
	handler.ServeHTTP(rr, req)
...
func TestHandleGiteaCommentWebhookIgnoredProject(t *testing.T) {
	settings.Gitea = settings.GiteaConfig{
		Webhook: &settings.Webhook{
			Secret: "",
		},
...
func TestHandleGiteaSynchronizeWebhookSuccess(t *testing.T) {
	settings.Gitea = settings.GiteaConfig{
		Webhook: &settings.Webhook{
			Secret: "",
		},
...
func TestHandleGiteaSynchronizeWebhookInvalidJSONBody(t *testing.T) {
	settings.Gitea = settings.GiteaConfig{
		Webhook: &settings.Webhook{
			Secret: "",
		},
	...
	req, rr, handler := withValidGiteaSynchronizeRequestData(t, []byte(`{ "action": ["non-string-value-for-action"] }`))
	handler.ServeHTTP(rr, req)
...
func TestHandleGiteaSynchronizeInvalidWebhookSignature(t *testing.T) {
	settings.Gitea = settings.GiteaConfig{
		Webhook: &settings.Webhook{
...
func TestHandleGiteaSynchronizeWebhookIgnoredProject(t *testing.T) {
	settings.Gitea = settings.GiteaConfig{
		Webhook: &settings.Webhook{
			Secret: "",
		},
